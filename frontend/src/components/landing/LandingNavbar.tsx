...
export const LandingNavbar: React.FC<LandingNavbarProps> = ({ onOpenDemo, loadingDemo }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between flex-nowrap gap-4">
        ...
        <Link to="/" className="flex items-center space-x-3 group shrink-0">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400 tracking-wider uppercase font-semibold whitespace-nowrap">
        ...
        <nav className="hidden xl:flex items-center space-x-1 whitespace-nowrap">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="px-3 py-1.5 text-xs lg:text-sm font-medium text-slate-300 hover:text-emerald-400 hover:bg-slate-900/60 rounded-lg transition-colors whitespace-nowrap"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden sm:flex items-center space-x-2.5 shrink-0 whitespace-nowrap">
          <a
            ...
            className="hidden md:flex items-center space-x-1.5 px-3 py-2 text-xs font-semibold text-emerald-300 bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-800/60 rounded-xl transition-all shadow-sm"
          >
          ...
          <button
            ...
            className="flex items-center space-x-2 px-3.5 py-2 text-xs font-semibold text-white bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 rounded-xl shadow-lg shadow-emerald-600/25 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
          >
            <Sparkles size={15} />
            <span>{loadingDemo ? 'Iniciando...' : 'Demo Grátis'}</span>
            <ArrowRight size={14} />
          </button>
        </div>
